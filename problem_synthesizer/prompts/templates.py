...
题型分布 (严格执行，缺一不可):
- 其中 {academic_count} 道为【学术知识考察题】(question_type: "academic")：
  考察纯理论、底层原理、协议细节、算法定义等，题干不含具体业务背景。
- 其中 {business_count} 道为【业务情景应用题】(question_type: "business_scenario")：
  设定一个真实工程或产品场景（如：某电商秒杀系统、某 LLM 推理服务），
  候选人需在该场景约束下选出正确的技术决策或判断系统行为。

Constraint (非常严格，必须遵守):
1. 选项数量：每道题固定提供 4 个选项（A, B, C, D）。
2. 正确项分布：每道题【必须有 2 到 3 个正确选项】，不能是单选题，也不能全对。
3. 每道题必须包含 "tag" 字段，值必须是上述知识点列表中的一项原文。
4. 每道题必须包含 "question_type" 字段，值为 "academic" 或 "business_scenario"。
5. 混淆设计原则（陷阱）：
   - 生成一个看似正确但实则违背底层原理的选项。
   - 生成一个在特定场景下才成立，但在题目描述的一般场景下不适用的选项（张冠李戴）。
6. 必须输出合法的 JSON 数组格式，不要用 Markdown 的 ```json 标签包裹。
